...
public class ClientPacketListener implements PacketListener {
    @PacketHandler(type = PacketType.NODE_CONNECTED)
    ...
    @PacketHandler(type = PacketType.NODE_DISCONNECTED)
    ...
    @PacketHandler(type = PacketType.SERVICE_DISCONNECTED)
    ...
    @PacketHandler(type = PacketType.SERVICE_CONNECTED)
    ...
    @PacketHandler(type = PacketType.CLUSTER_STOPPED)
    ...
    @PacketHandler(type = PacketType.SERVICE_INFO_UPDATED)
    ...
    @PacketHandler(type = PacketType.GROUP_INFO_UPDATED)
    ...
    public void onStartService(Packet packet){
        ...
        Optional<IService> optionalIService = Cloud.getModule().getComponent(IServiceManager.class).getService(serviceId);
        if (optionalIService.isPresent()){
            IService service = optionalIService.get();
            if (Cloud.getWrapper().isThisModule(service.getNode())){
                Cloud.getModule().getComponent(IServiceManager.class).start();
            }
        }
    }
}
